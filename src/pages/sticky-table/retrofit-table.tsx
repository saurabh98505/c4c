...
import { COLUMNS } from './retrofit-column';
...
export const RetroTableDataNew = (props: any) => {
    ...
    const commonCall = async () => {
        ...
        setShowLoader(true);
        await axios.get(
            'https://eu-api.jotform.com/user?apiKey=45c0e9b8e1a55bb227fb1b5510492565&limit=999'
        );
        const data: any = await axios.get(
            'https://eu-api.jotform.com/form/230212862269353/submissions?apiKey=45c0e9b8e1a55bb227fb1b5510492565&limit=999'
        );

        // const data1: any = await axios.get('http://c4c-cen.com/api/getAllShop');
        ...
        if (response && Array.isArray(response)) {
            sanitizedArray = response.map((r: any) => {
                const mainObj: any = {};
                ...
                return {
                    ...
                    status: r?.status,
                    geben_sie_die_tankstellennummer_ein: `${r?.answers[86]?.answer}`,
                    strasse: `${r?.answers[87]?.answer}`,
                    ort: `${r?.answers[88]?.answer}`,
                    //need
                    timer: `${r?.answers[54]?.answer ?? ''}`,
                    geo: `${r?.answers[64]?.answer ?? ''}`,
                    retro_id: `${r?.answers[49]?.answer ?? ''}`,
                    servers: `${r?.answers[7]?.answer ?? ''}`,
                    abmessugen: `${r?.answers[8]?.answer ?? ''}`,
                    foto_upload: r.answers[35]?.answer ?? '',

                    //no
                    gesamtlänge_kontur_austausch: `${
                        r?.answers[119]?.answer ?? ''
                    }`,
                    freigabe_fertigung: `${r?.answers[109]?.answer}`,
                    handlungsbedarf_vorhanden: `${r?.answers[118]?.answer}`,
                    projekt_abschluss: `${r?.answers[114]?.answer}`,
                    datensatz_geprüft: `${r?.answers[108]?.answer}`,
                    fehlende_daten: `${r?.answers[116]?.answer}`,
                    geplante_fertigung: `${r?.answers[110]?.answer ?? ''}`,
                    geplante_montage: `${r?.answers[111]?.answer ?? ''}`,
                    ist_ferigung: `${r?.answers[112]?.answer ?? ''}`,
                    ist_montage: `${r?.answers[113]?.answer ?? ''}`,
                    höhe_der_kontur: `${r?.answers[99]?.maxValue ?? ''}`,
                    länge_der_kontur: `${r?.answers[100]?.answer ?? ''}`,
                    rück_kantung_der_kontur: `${r?.answers[101]?.answer ?? ''}`,
                    datum: `${r?.answers[41]?.prettyFormat ?? ''}`,
                    insp_nr: `${r?.answers[50]?.answer ?? ''}`,
                    id: `${r?.answers[49]?.answer ?? ''}`,
                    ausführendes_unternehmen: `${
                        r?.answers[115]?.answer ?? ''
                    }`,
                    mind_4_Fotos_der_Fassadenbereiche_umlaufend: `${
                        r?.answers[102]?.answer ?? ''
                    }`,
                    abmessungen_von_rückspringenden_fassadenbereichen_mit_abkantung: `${
                        r.answers[104]?.prettyFormat ?? ''
                    }`,
                    eingangsbox_vorhanden: r.answers[6]?.answer ?? '',
                    anzahl_Blech_Lagen_übereinander: r.answers[7]?.answer ?? '',
                    ist_zustand_der_konturbefestigung:
                        r.answers[16]?.answer ?? '',
                    befestigungs_untergrund: r.answers[17]?.answer ?? '',
                    sonstige_bemerkungen: r.answers[32]?.answer ?? '',
                    aufmaß_skizze_bitte_hier_hochladen:
                        r.answers[35]?.answer ?? '',
                    datei_upload: r.answers[37]?.answer ?? '',
                    name_des_verantwortlichen_monteurs:
                        r.answers?.[40]?.answer?.first +
                            ' ' +
                            r.answers?.[40]?.answer?.last ?? '',
                    datum2: r.answers[41]?.answer.datetime ?? '',
                    unterschrift: r.answers[42]?.answer ?? '',
                    bei_kontur_demontage_zurückzubauende_ausrüstungsgegenstände: `${
                        r.answers[105]?.prettyFormat ?? ''
                    }`,
                    aei: r.answers[104] ?? '',
                    bei: r.answers[105] ?? '',
                    form_id: r?.id,
                    ...
                    montierteLosung: r.answers[98].prettyFormat ?? '',
                };
            });
        }

        let mapArray = [...sanitizedArray];
        if(props.station_id){
            mapArray = mapArray.filter(stationData => stationData.geben_sie_die_tankstellennummer_ein.includes(props.station_id))
        }
        const sanitizedArrayCSV = mapArray.map((r: any) => {
            const mainObj: any = {};
            return {
                ...r,
                abmessungen_von_rückspringenden_fassadenbereichen_mit_abkantung: `${
                    convert(
                        r.abmessungen_von_rückspringenden_fassadenbereichen_mit_abkantung
                    ) ?? ''
                }`,
                bei_kontur_demontage_zurückzubauende_ausrüstungsgegenstände: `${
                    convert(
                        r.bei_kontur_demontage_zurückzubauende_ausrüstungsgegenstände
                    ) ?? ''
                }`,
            };
        });

        setDataCsv(sanitizedArrayCSV);
    ...
    const imageClickHandler = (images) => {
        ...
        if (Array.isArray(imageMind4) && imageMind4.length > 0) {
            imageMind4.map((img) => {
                if (getExtension(img).toLowerCase() === 'pdf') {
                    isPdf = true;
                }
            });
        }
        if (Array.isArray(imageMind4) && imageMind4.length > 0 && !isPdf) {
            setSliderImages(
                imageMind4.map((img) => {
                    if (getExtension(img).toLowerCase() !== 'pdf') {
                        return {
    ...
    function convertImageToBase64(imgUrl, callback) {
        // console.log(imgUrl, 'imgUrlother');
        // if (!imgUrl.match(/\.(jpg|jpeg|png|gif)$/i)) {
        //     setErrorPdf('Invalid Inputs in data');
        //     setShowButtonLoader(false);
        // }
    ...
    function convertImageToBase64Paper(imgUrl, callback) {
        // if (!imgUrl.match(/\.(jpg|jpeg|png|gif)$/i)) {
        //     setErrorPdf('Invalid Inputs in data');
        //     setShowButtonLoader(false);
        // }
    ...
    const exportPdf = async () => {
        ...
        // if(data[key[0]]?.is_edit === false){

        //     const dataPDf: any = await axios.get(
        //         `https://www.jotform.com/server.php?action=getSubmissionPDF&sid=${data[key[0]]?.form_id}&formID=230423415414343`
        //     );
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Aufnahme Datum', 10, 60, { maxWidth: 30 });
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Freigabe Fertigung', 100, 60, { maxWidth: 40 });
        doc.setFont('helvetica', 'normal');
        doc.text(data[key[0]]?.freigabe_fertigung, 150, 60, { maxWidth: 40 });

        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Freigabe-Status', 10, 70, { maxWidth: 30 });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.text(data[key[0]]?.status, 60, 70, { maxWidth: 30 });

        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Fehlende Daten', 100, 70, { maxWidth: 40 });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.text(data[key[0]]?.fehlende_daten, 150, 70, { maxWidth: 40 });

        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Tankstellen-Nr.', 10, 80, { maxWidth: 30 });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.text(data[key[0]]?.geben_sie_die_tankstellennummer_ein, 60, 80, {
            maxWidth: 30,
        });

        doc.setFont('helvetica', 'bold');
        doc.text('Bestandsaufnahme durch', 100, 80, { maxWidth: 40 });
        doc.setFont('helvetica', 'normal');
        doc.text(data[key[0]]?.ausführendes_unternehmen, 150, 80, {
            maxWidth: 40,
        });

        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Strasse', 10, 90, { maxWidth: 30 });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.text(data[key[0]]?.strasse, 60, 90, { maxWidth: 30 });
        ...
        if (data[key[0]]?.is_edit === false) {
            ...
            doc.table(10, 150, mainValuess, keyss, {
                autoSize: true,
                fontSize: 6,
            });
            let keys: any[] = [' '];
            ...
            doc.table(100, 150, mainValues, keys, {
                autoSize: true,
                fontSize: 6,
            });
        }
        ...
        for (let i of data[key[0]]?.aufmaß_skizze_bitte_hier_hochladen) {
            if (!i.match(/\.(jpg|jpeg|png|gif)$/i)) {
                await fetch(i).then((response) => {
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.text('Mind. 4 Fotos der Fassadenbereiche umlaufend', 10, 160);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        j = 0;
        const images =
        ...
        for (let i of images) {
            ...
            let img: any = await convertImageToBase64(i, console.log);
        ...
        await doc.save('frn_retrofit_shelf.pdf');
        setErrorPdf('');
        setShowButtonLoader(false);
    };
                        ...
                        {dataLogin?.loginType === 'admin' && (
                            ...
                            >
                                <CSVLink
                                    data={dataCsv}
                                    headers={csvColumn}
                                    filename={'frn_retrofit_shelf.csv'}
                                ...
                                <div>
                                    {alert && (
                                        <Alert severity="warning">
                                            Edit is not allowed for this data
                                        </Alert>
                                    )}
                                    <Autocomplete
                                        value={value}
