...
import { COLUMNS } from './shop-column';
...
export const ShopTableData = (props) => {
    ...
    const commonCall = async () => {
        ...
        const data: any = await axios.get(
            'https://eu-api.jotform.com/form/230423415414343/submissions?apiKey=45c0e9b8e1a55bb227fb1b5510492565&limit=999'
        );
        let response1 = [];
        if (props?.station_id && props.station_id) {
            ...
            const data1: any = await axios.post(
                'https://c4c-cen.com/api/getAllShop',
                formdata,
                {
                    headers: {
        ...
        if (response && Array.isArray(response)) {
            sanitizedArray = response.map((r: any) => {
                ...
                return {
                    ...
                    ausführendes_unternehmen: `${r.answers[115]?.answer ?? ''}`,
                    mind_4_Fotos_der_Fassadenbereiche_umlaufend: `${
            ...
            if(props.station_id){
                sanitizedArray = sanitizedArray.filter(stationdata => stationdata.geben_sie_die_tankstellennummer_ein.includes(props.station_id))
            }
        }
        let sanitizedArray1: any = [];
        if (response1 && Array.isArray(response1)) {
            sanitizedArray1 = response1.map((r: any) => {
                const mainObj: any = {};
        ...
        const mapArray = [...sanitizedArray, ...sanitizedArray1];
    ...
    const exportPdf = async () => {
        ...
        await doc.save('shop_fassden_check.pdf');
        setErrorPdf('');
        setShowButtonLoader(false);
    };
